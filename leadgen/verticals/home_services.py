"""
Vertical: home-services / trades web leads.

Targets local trades — plumbers, electricians, HVAC, roofers, carpenters,
landscapers. Same buyer logic as web_design (no site, social-only, or a
weak/slow DIY site = a good web-design prospect), but the pitch is tuned to how
trades actually get work: ranking for '<trade> <city>' so they show up when a
homeowner searches in an emergency.
...
def _opener(rec: dict) -> str:
    ...
    trade = rec.get("category") or "contractor"
    city = rec.get("city") or ""
    if not site:
        return (f"No website — homeowners can't find them in an emergency. Pitch a "
                f"site that ranks for '{trade} {city}'.")
    if "facebook" in site or "instagram" in site:
        return (f"Only on social — pitch an owned site that ranks for "
                f"'{trade} {city}' on Google.")
    if site.startswith("http://"):
        return "HTTP only — Chrome flags it 'Not secure'. Quick rebuild + SSL."
    return f"Has a site — check whether it ranks for '{trade} {city}' before pitching."
...
register(Vertical(
    key="home_services",
    label="Home-service trades that need a website",
    description=("Finds plumbers, electricians, HVAC, roofers, and landscapers "
                 "with no/weak/slow site — web-design prospects that need to rank "
                 "for '<trade> <city>'."),
    overture_categories=["plumber", "electrician", "hvac", "roofer",
                         "landscap", "contractor"],
    osm_tags=["craft=plumber", "craft=electrician", "craft=hvac",
              "craft=roofer", "craft=carpenter", "shop=trade", "craft=gardener"],
    keep_chains=False,
    score_fn=_score,
